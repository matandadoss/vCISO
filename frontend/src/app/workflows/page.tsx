...
export default function WorkflowsPage() {
  ...
  const fetchWorkflows = async () => {
    try {
      const res = await fetchWithAuth("http://localhost:8000/api/v1/workflows");
  ...
  const handleToggle = async (id: string) => {
    try {
      const res = await fetchWithAuth(`http://localhost:8000/api/v1/workflows/${id}/toggle`, { method: "POST" });
      if (res.ok) {
  ...
  const handleRun = async (id: string) => {
    try {
      const res = await fetchWithAuth(`http://localhost:8000/api/v1/workflows/${id}/run`, { method: "POST" });
      if (res.ok) {
