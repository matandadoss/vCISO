...
import { Activity, AlertCircle, Cpu, Network, ShieldAlert, ChevronDown, ChevronUp, Info, FileText } from "lucide-react";
         ...
         <div className="max-w-7xl mx-auto space-y-8">
            ...
            <Tabs defaultValue="overview" className="space-y-6 w-full">
               <TabsList className="h-auto flex-wrap justify-start">
                  ...
                  <TabsTrigger value="report" className="flex items-center gap-2"><FileText className="w-4 h-4" /> Intel Briefing</TabsTrigger>
               </TabsList>
            ...
            <div>
               ...
               <TabsContent value="report" className="mt-2 text-foreground space-y-6">
                   <div className="bg-card border border-border rounded-lg p-8 shadow-sm">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-border pb-6 mb-6">
                         <div>
                            <h2 className="text-2xl font-bold flex items-center gap-2 tracking-tight"><FileText className="w-6 h-6 text-primary" /> Executive Threat Intelligence Briefing</h2>
                            <p className="text-muted-foreground mt-1 text-sm">Automated Aggregation of OSINT & Deep Web Correlation</p>
                         </div>
                         <div className="text-left md:text-right mt-4 md:mt-0">
                            <div className="text-sm font-bold text-foreground">{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                            <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Generated by Virtual CISO</div>
                         </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                         <div className="bg-card border border-border rounded-lg p-5 shadow-sm">
                            <div className="text-xs text-muted-foreground uppercase tracking-wider font-bold mb-1">Total Threats Monitored</div>
                            <div className="text-3xl font-light text-foreground">{metrics?.active_patterns || "--"}</div>
                         </div>
                         <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-5 shadow-sm">
                            <div className="text-xs text-red-500/80 uppercase tracking-wider font-bold mb-1">Critical Action Paths</div>
                            <div className="text-3xl font-light text-red-500">{metrics?.critical_risk_paths || "--"}</div>
                         </div>
                         <div className="bg-primary/5 border border-primary/20 rounded-lg p-5 shadow-sm">
                            <div className="text-xs text-primary/80 uppercase tracking-wider font-bold mb-1">External Feeds Scanned</div>
                            <div className="text-3xl font-light text-primary">{metrics?.evaluated_workflows || "--"}</div>
                         </div>
                      </div>

                      <div className="space-y-6">
                         <h3 className="text-lg font-bold border-b border-border pb-2">Top Active Threats Targeting Our Ecosystem</h3>
                         {correlations.length === 0 ? (
                            <p className="text-muted-foreground text-sm italic">No active threats detected against your technology stack.</p>
                         ) : (
                               ...
                               <table className="w-full text-left text-sm border-collapse">
                                  <thead className="sticky top-0 z-10">
                                     <tr className="border-b border-border bg-muted text-xs uppercase tracking-wider text-muted-foreground">
                                        ...
                                        <th className="py-3 px-4 font-bold">Targeted Asset</th>
                                        <th className="py-3 px-4 font-bold">Threat Indicator / Description</th>
                                     </tr>
                                  </thead>
                                  <tbody className="divide-y divide-border bg-card">
                                     {correlations.map(corr => (
                                        <tr key={corr.id} className="hover:bg-muted/50 transition-colors">
                                           <td className="py-3 px-4 whitespace-nowrap">
                                              <span className="font-bold flex items-center gap-1.5" style={{ color: corr.progress_color }}>
                                                 <Activity className="w-3.5 h-3.5" />
                                                 {corr.severity_tag}
                                              </span>
                                           </td>
                                           <td className="py-3 px-4 font-medium text-foreground">{corr.target_name}</td>
                                           <td className="py-3 px-4">
                                              <span className="font-semibold block mb-0.5">{corr.impact_summary}</span>
                                              <span className="text-muted-foreground text-xs line-clamp-2">{corr.external_signal}</span>
                                           </td>
                                        </tr>
                                     ))}
                                  </tbody>
                      ...
                      <div className="mt-8 pt-6 border-t border-border">
                         <h3 className="text-lg font-bold mb-3 text-primary flex items-center gap-2"><Cpu className="w-5 h-5" /> Analyst Recommendation Summary</h3>
                         <div className="bg-primary/5 border border-primary/20 rounded-lg p-5 text-sm leading-relaxed text-foreground">
                            Based on cross-correlation between Global OSINT and internal technology graphs, the Immediate Action focus should remain on hardening <strong>{correlations[0]?.target_name || "core perimeter endpoints"}</strong> against <strong>{correlations[0]?.source_name || "external vulnerability"}</strong> tracking. Current mitigation effectiveness is measured at <span className="font-bold font-mono text-primary bg-primary/10 px-1.5 py-0.5 rounded">{metrics?.avg_control_effectiveness || "--"}</span>.
                         </div>
                      </div>
                   </div>
                </TabsContent>
